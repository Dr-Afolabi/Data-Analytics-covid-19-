...
worldbar <-Worlddatanew%>% ggplot(aes(reorder(x=countriesAndTerritories,deathpop), 
  ...
  xlab("Countries")+
  ylab("Covid-19 Death population per million")+
  #legend(title = "Countries")+
  theme_bw()+
  ggtitle("Death population 4 sm countries")+
  guides(fill = F)+
...
# Formating dates?
Worlddatanew1 <- Worlddatanew %>%mutate(dateRep  = dmy(dateRep))
